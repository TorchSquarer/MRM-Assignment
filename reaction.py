...
class ReactionRates:
    ...
    def reaction_rates(self, c_p: np.ndarray, cfg: ModelConfig) -> tuple[np.ndarray, np.ndarray]:
        ...
        alpha_2 = P_CO2 * P_CH3OH**2 - (P_DMC * P_H2O) / self.cfg.k_eq_r2()
        inhibition_r2 = (1.0 + self.cfg.k_ads1 * P_CH3OH + self.cfg.k_ads2 * P_CH3OH * P_CO2) ** 2
        r2 = self.cfg.rho_bulk * self.cfg.k_eff_r2() * alpha_2 / inhibition_r2
    ...
    def particle_reaction_rates(self, c_p: np.ndarray, cfg: ModelConfig) -> np.ndarray:
        ...
        return np.einsum("zrc,co->zrc", rates, STOICH)